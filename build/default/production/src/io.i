...
# 1 "src/io.c" 2
# 54 "src/io.c"
# 1 "src/../src/supermez80.h" 1
# 10 "src/../src/supermez80.h"
# 1 "src/../src/picconfig.h" 1
# 10 "src/../src/supermez80.h" 2
...
# 14 "src/../src/supermez80.h" 2
# 48 "src/../src/supermez80.h"
union address_bus_u {
 unsigned int w;
 struct {
  unsigned char l;
...
# 1 "src/chk_borad_dpend.h" 1
# 100 "src/../src/supermez80.h" 2
# 54 "src/io.c" 2
...
# 1 "src/../drivers/SDCard.h" 1
# 46 "src/../drivers/SDCard.h"
...
# 1 "src/../drivers/SPI.h" 1
# 36 "src/../drivers/SPI.h"
enum {
    SPI_CLOCK_100KHZ,
    SPI_CLOCK_2MHZ,
    SPI_CLOCK_4MHZ,
...
void SPI_PREFIX_begin(struct SPI *ctx_);
void SPI_PREFIX_configure(struct SPI *ctx_, int clock_speed, uint8_t bitOrder, uint8_t dataMode);
void SPI_PREFIX_begin_transaction(struct SPI *ctx_);
uint8_t SPI_PREFIX_transfer_byte(struct SPI *ctx_, uint8_t output);
void SPI_PREFIX_transfer(struct SPI *ctx_, void *buf, unsigned int count);
void SPI_PREFIX_send(struct SPI *ctx_, const void *buf, unsigned int count);
void SPI_PREFIX_receive(struct SPI *ctx_, void *buf, unsigned int count);
void SPI_PREFIX_end_transaction(struct SPI *ctx_);
void SPI_PREFIX_dummy_clocks(struct SPI *ctx_, unsigned int clocks);
uint8_t SPI_PREFIX_receive_byte(struct SPI *ctx_);
void SPI_PREFIX_select(struct SPI *ctx_, int select);
extern struct SPI *SPI_PREFIX_ctx;
# 58 "src/io.c" 2
...
int menu_select(void)
{
 ...
 for (drive = 0; drive < num_drives && num_files < 6; drive++) {
...
void mem_init()
{
    ...
    for (addr = 0; addr < 0x00100000; addr += 256 * 16) {
        printf("Memory 000000 - %06lXH\r", addr);
        tmp_buf[0][0] = (addr >> 0) & 0xff;
        tmp_buf[0][1] = (addr >> 8) & 0xff;
        tmp_buf[0][2] = (addr >> 16) & 0xff;
        emuz80_57q_write_to_sram((uint16_t)addr, tmp_buf[0], 256);
        emuz80_57q_read_from_sram((uint16_t)addr, tmp_buf[1], 256);
        if (memcmp(tmp_buf[0], tmp_buf[1], 256) != 0) {
            break;
        }
        if (addr == 0)
            continue;
        emuz80_57q_read_from_sram(0, tmp_buf[1], 256);
        if (memcmp(tmp_buf[0], tmp_buf[1], 256) == 0) {
...
void SD_init()
{
 ...
 for (retry = 0; 1; retry++) {
  ...
  if (SDCard_init(SPI_CLOCK_100KHZ, SPI_CLOCK_8MHZ, 100) == 0)
   break;
  _delay((unsigned long)((200)*(64000000UL/4000.0)));
 }
}
...
void disk_io_handle() {
 ...
 static uint8_t *disk_datap = ((void*)0);


 uint32_t sector = 0;
 disk_drive = emuz80_57q_get_from_sram(0xfb20);
    disk_track = emuz80_57q_get_from_sram(0xfb21);
 disk_sector = (disk_sector & 0xff00) | emuz80_57q_get_from_sram(0xfb22);
 disk_dmal = emuz80_57q_get_from_sram(0xfb23);
 disk_dmah = emuz80_57q_get_from_sram(0xfb24);

 if (num_drives <= disk_drive || drives[disk_drive].filep == ((void*)0)) {
 ...
 if (io_addr == 0x40 +18) {
  ...
  emuz80_57q_write_to_sram(addr, (uint8_t*)disk_buf, 128);
  disk_datap = ((void*)0);
 ...
 if (io_addr == 0x40 +19) {
  ...
  uint16_t addr = ((uint16_t)disk_dmah << 8) | disk_dmal;
  emuz80_57q_read_from_sram(addr, (uint8_t*)disk_buf, 128);
