...
int util_stricmp(const char *a, const char *b)
{
  int ua, ub;
  do {
      ua = ((((unsigned char)(((unsigned char)*a++)-'a')) < 26) ? ((unsigned char)*a++)&0x5f : ((unsigned char)*a++));
      ub = ((((unsigned char)(((unsigned char)*b++)-'a')) < 26) ? ((unsigned char)*b++)&0x5f : ((unsigned char)*b++));
   } while (ua == ub && ua != '\0');
   return ua - ub;
}
